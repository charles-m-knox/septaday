...
export const initializeDB = (db: SQLite.WebSQLDatabase, tasks: Task[], setTasks: React.Dispatch<React.SetStateAction<Task[]>>): void => {
    if (!db) return;
    db.transaction(tx => {
...
export const pushTasksToDB = (db: SQLite.WebSQLDatabase, tasks: Task[], callback: any): void => {
    if (!db) return;
    const dateInt = getDateInt();
...
export const resetDB = (db: SQLite.WebSQLDatabase) => {
    if (db) {
...
export const getTaskHistoryFromDB = (db: SQLite.WebSQLDatabase, setTasks: React.Dispatch<React.SetStateAction<Task[]>>) => {
    if (!db) return;
...
export const getTaskHistoryTx = (tx: SQLite.SQLTransaction, setTasks: React.Dispatch<React.SetStateAction<Task[]>>) => {
    ...
    tx.executeSql(getTaskHistorySQL, [dateInt], (tx: SQLTransaction, resultSet: SQLResultSet): void => {
        ...
        setTasks(tasksFromTx.map((task: any) => {
